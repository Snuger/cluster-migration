...
# 获取所有Deployment名称
all_deployments=$(kubectl get deploy -n "$SOURCE_NS" -o=jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' 2>/dev/null)
# 应用过滤
...
for deployment in $deployments; do  
    if [ -n "$deployment" ]; then  # 确保不为空
        ...
        # 获取Deployment YAML
        temp_yaml=$(kubectl get deploy "$deployment" -n "$SOURCE_NS" -o yaml | \
...
all_services=$(kubectl get svc -n "$SOURCE_NS" -o=jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' 2>/dev/null | grep -v '^kubernetes$')
# 应用过滤
...
all_vs=$(kubectl get virtualservices.networking.istio.io -n "$SOURCE_NS" -o=jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' 2>/dev/null)
# 应用过滤
...
for vs in $source_vs; do
    if [ -n "$vs" ]; then  # 确保不为空
        ...
        temp_yaml=$(kubectl get virtualservices.networking.istio.io "$vs" -n "$SOURCE_NS" -o yaml | \
        sed -e '/resourceVersion:/d' -e '/uid:/d')
...
all_dr=$(kubectl get destinationrules.networking.istio.io -n "$SOURCE_NS" -o=jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' 2>/dev/null)
# 应用过滤
...
for dr in $source_dr; do
    if [ -n "$dr" ]; then  # 确保不为空
        ...
        temp_yaml=$(kubectl get destinationrules.networking.istio.io "$dr" -n "$SOURCE_NS" -o yaml | \
        sed -e '/resourceVersion:/d' -e '/uid:/d')
